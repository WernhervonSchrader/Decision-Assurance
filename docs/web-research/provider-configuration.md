...
`BRAVE_API_KEY` enables discovery and `FIRECRAWL_API_KEY` enables extraction. The names are secret
references, not permission to expose values as ordinary configuration. Local development should set
`DA_SECRET_DIRECTORY=.secrets` and create `.secrets/BRAVE_API_KEY` and
...
Real Brave and Firecrawl hosts are enabled locally only with
...
bounded reason code. URLs, headers, bodies, result content and credentials are excluded. Brave and
