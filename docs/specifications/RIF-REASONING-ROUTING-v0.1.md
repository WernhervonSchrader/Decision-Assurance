...
The RIF orchestrator selects a reasoning workflow from a finite, policy-approved route set and
...
1. Authenticate the caller; derive tenant, actor, permitted workflow, locale and data classification
   from trusted context and policy. Verify request schema, size, canonical digest and idempotency.
...
5. Persist a redacted append-only routing record before dispatch. Dispatch exactly the effective
   route. Failure to record a required event prevents automated dispatch.
...
```json
{
  ...
  "snapshot_digest": "sha256:<hex-of-minimized-canonical-input>",
  "candidate_set_digest": "sha256:<hex-of-canonical-options-and-criteria>",
  "candidate_routes": ["FAST_REASONING", "FULL_REASONING", "SPECIALIST_REASONING", "HUMAN_REVIEW"],
...
from selector output. `snapshot_digest` binds the exact approved, minimized input, including
classification and locale metadata where routing relevant; it is not a license to retain raw
content. `candidate_set_digest` binds route labels, meanings, capability constraints and policy
criteria, not just the array order. Define a canonical JSON algorithm and domain separator before
computing either hash. `proposed_route` is the untrusted selector result; `effective_route` is the
policy outcome. The persisted contract records both, including explicit abstention or outage
reason codes when no valid proposal exists; the schema must then allow `proposed_route: null`.

`signal.probabilities` is optional and must either cover exactly the eligible candidate set with
...
| Actor independence and audit | implicit approval | distinct DA authority, append-only route events | design only | bypass and audit failure tests | DA owner | NOT TESTED |
...
1. Confirm the actual RIF/DA integration boundary and event registry mapping in the target
   implementation revision; existing Web Research egress controls are a pattern, not evidence that
   routing already uses them.
2. Establish route eligibility, specialty identifiers, blocked outcome semantics, canonicalization
   algorithm, audit retention and performance/cost budgets.
