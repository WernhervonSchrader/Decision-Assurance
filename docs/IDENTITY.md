...
The Keycloak profile requires `sub`, `tenant_id`, `realm_access.roles`, `actor_kind`, `azp` and
`da.api` scope, in addition to `iss`, `aud`, `iat` and `exp`. `nbf` is verified whenever present.
Organization and groups are optional, explicitly configured mappings.
Groups must be a bounded list of non-empty strings.

Supported mapped roles are Generator, Validator, Approver, Auditor, Research Operator, Readonly,
Tenant Administrator and System Administrator. System Administrator receives only cross-cutting report
...
The supported local realm, clients, role mapping, PKCE flow and production gate are documented in
