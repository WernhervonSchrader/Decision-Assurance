...
Introduce a versioned, strict machine-readable `reasoning_route_decision` owned by RIF. An
...
- Record a route decision and egress decision with correlation ID, policy and selector versions,
  candidate-set and sanitized-input digests, tenant scope, reason codes and effective route.
  Preserve audit append-only semantics and avoid storing raw task text or credentials by default.
