...
def _repository(request: Request) -> DecisionRepository:
...
async def create_decision(
    ...
    try:
        _repository(request).create_decision(
            identity.tenant,
            document,
            [event],
            _idempotency_write(identity, key, "decision:create", digest, 201, document),
        )
...
def evaluate(
    ...
    body: dict[str, Any] = result.report
    operation = f"decision:evaluate:{decision_id}"
    _repository(request).save_result(
        identity.tenant,
        updated,
        body,
        new_events,
        _idempotency_write(identity, key, operation, digest, 200, body),
    )
    return body
...
def get_audit(
    ...
    return {
        "items": _repository(request).list_audit(
            identity.tenant, decision_id, limit=limit, offset=offset
        ),
        "limit": limit,
        "offset": offset,
    }
