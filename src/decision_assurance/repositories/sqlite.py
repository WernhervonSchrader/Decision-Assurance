...
class SqliteDecisionRepository:
    def __init__(self, database_path: Path, migration_path: Path | None = None):
        ...
        self.migration_path = migration_path or Path(__file__).parents[3] / "migrations" / "001_api_v0_2.sql"
    ...
    def save_result(
        ...
        document_json = json.dumps(document, sort_keys=True, separators=(",", ":"), ensure_ascii=False)
        with self._connect() as connection:
            ...
            for event in events:
                connection.execute(
                    ...
                    (tenant.tenant_id, decision_id, event["event_id"], json.dumps(event, sort_keys=True, separators=(",", ":"), ensure_ascii=False)),
                )
    ...
    def store_idempotency(
        self, tenant: TenantContext, actor_id: str, operation: str, key: str,
        request_hash: str, status_code: int, response: dict[str, Any]
    ) -> None:
        with self._connect() as connection:
            connection.execute(
                "INSERT INTO idempotency (tenant_id, actor_id, operation, idempotency_key, request_hash, status_code, response_json) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (tenant.tenant_id, actor_id, operation, key, request_hash, status_code, json.dumps(response, sort_keys=True, separators=(",", ":"), ensure_ascii=False)),
            )
