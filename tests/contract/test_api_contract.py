...
def test_create_is_strict_and_idempotent(client: TestClient, decision: dict) -> None:
    ...
    assert (
        client.post(
            "/v1/decisions", headers=headers("a-generator", "create-1"), json=manipulated
        ).status_code
        == 409
    )
...
def test_unknown_fields_and_actor_spoofing_are_rejected(client: TestClient, decision: dict) -> None:
    ...
    assert (
        client.post("/v1/decisions", headers=headers("a-generator", "x2"), json=spoofed).status_code
        == 403
    )
...
def test_cross_tenant_case_is_not_enumerable(client: TestClient, decision: dict) -> None:
    assert (
        client.post(
            "/v1/decisions", headers=headers("a-generator", "x3"), json=decision
        ).status_code
        == 201
    )
    assert (
        client.get(
            f"/v1/decisions/{decision['decision_id']}", headers=headers("b-generator")
        ).status_code
        == 404
    )
...
def test_validator_can_evaluate_and_auditor_can_read(client: TestClient, decision: dict) -> None:
    ...
    evaluated = client.post(
        f"/v1/decisions/{decision_id}/evaluate", headers=headers("a-validator", "eval-1")
    )
    assert evaluated.status_code == 200
    assert evaluated.json()["outcome"] == "PASS"
    assert (
        client.get(f"/v1/decisions/{decision_id}/report", headers=headers("a-auditor")).json()[
            "outcome"
        ]
        == "PASS"
    )
    events = client.get(f"/v1/decisions/{decision_id}/audit", headers=headers("a-auditor")).json()[
        "items"
    ]
...
def test_wrong_role_and_agent_approval_fail_closed(client: TestClient, decision: dict) -> None:
    ...
    assert (
        client.post(
            f"/v1/decisions/{decision_id}/evaluate", headers=headers("a-generator", "eval-x")
        ).status_code
        == 403
    )
    ...
    client.post(
        ...
        headers=headers("a-validator", "tr-1"),
        json={"target": "VALIDATION"},
    )
    client.post(
        f"/v1/decisions/{decision_id}/transitions",
        headers=headers("a-validator", "tr-2"),
        json={"target": "REVIEW"},
    )
    response = client.post(
        f"/v1/decisions/{decision_id}/transitions",
        headers=headers("a-agent-approver", "tr-3"),
